...
%2
\item Nach dem Empfangen der \textit{N1 message} sendet die \gls{seaf} in Schritt 2 den \textit{Nausf\_UEAuthentication\_ Authenticate Request} an die \gls{ausf}.
Falls in der \textit{N1 message} der \gls{5g-guti} enthalten ist, dann soll daraus der \gls{supi} abgeleitet und an die \gls{ausf} gesendet werden, ansonsten wird der \gls{suci} weitergeleitet.
...
%3
\item Nach dem Empfangen des \textit{Nausf\_UEAuthentication\_Authenticate Request}s sendet die \gls{ausf} den \textit{Nudm\_UEAuthentication\_Get Request} an das \gls{udm}.
...
Sie beinhaltet den \gls{5g-he-av}, der in \textit{Schritt 6} generiert wurde.
...
%8
\item In Schritt 8 wird die \gls{xres*}, die in dem \gls{5g-he-av} aus \textit{Schritt 7} enthalten ist, zwischengespeichert, da sie in \textit{Schritt 16} f\"ur die Verifikation von \gls{res*} ben\"otigt wird.
...
%12
\item Nachdem das \gls{ue} den \textit{Authentication Request} von der \gls{seaf} erhalten hat wird in Schritt 12 aus der \gls{rand} und dem \gls{autn} die \gls{res*} f\"ur die \textit{Authentication Response} berechnet.
...
Stimmen sie \"ubereinn so wird bei \textit{Schritt 15} weitergemacht.
...
Er beinhaltet die \gls{res*} aus der \textit{Authentication Response}.
...
Stimmen sie \"ubereinn so wird bei \textit{Schritt 17} weitergemacht.
...
%17
\item In Schritt 17 sendet die \gls{ausf} die \textit{Nausf\_UEAuthenticarion\_Authenticate Response} an die \gls{seaf}.
Sie beinhaltet das Ergebnis des Vergleichs von \gls{res*} und \gls{xres*} aus \textit{Schritt 16} und den \textit{''Anchor Key''} \gls{k-seaf}.
Wenn die \gls{ausf} in \textit{Schritt 7} den \gls{supi} erhalten hat, dann wird dieser nun auch an die \gls{seaf} weitergeleitet.
\end{enumerate}
